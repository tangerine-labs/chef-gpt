...
- **Prompts** live in `server/prompts.ts`: one user message per routine that names the tools in order. Completion callbacks run without the caller's token, so prompt arguments cannot autocomplete from the database. Probe with `deno task mcp prompts` / `prompt <name>`.
