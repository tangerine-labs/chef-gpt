...
- **Views are split**: presentational components in `packages/ui/views/` (props in, callbacks out, no MCP hooks) and thin hook wrappers in `server/views/*/view.tsx`. Put layout/state logic in the component so the fixture page can render it.
- **Check layout and states without a backend**: `deno task preview` opens `site/preview.html` (every component in several states from `site/src/fixtures.ts`; `?story=…&theme=dark` narrows it), `deno task preview:snap [--story "Vote"] [--dark] [--width 400]` renders it headlessly to a PNG. Add a story when you add a state. Three tiers: preview (layout/states) → `snap` (real data, stills) → Inspector + Chrome (real interaction).
- **See a view with `deno task snap <tool> ['{json}'] [--dark] [--out png]`** — headless render of the deployed view as the test user (wraps `mcp-use screenshot`); then Read the PNG. Seed the test household first with `deno task mcp call …` so the panel has data.
- **A look-judgement is a screenshot.** Whenever the question is whether something looks good, right, or fixed, run `deno task snap <tool>` (deployed view) or `deno task preview:snap --story …` (fixture) and Read the PNG before answering; reading the component source is not evidence. PRs that touch views get the same set posted as a comment by `.github/workflows/snapshots.yml` (`preview:snap --all`, every story, light + dark).
- **Click through a view live**: `deno task dev` (needs `server/package.json`, metadata only). The task pins `MCP_URL`/`MCP_ASSETS_URL`/`SITE_ORIGIN` to localhost; with the deployed `MCP_URL` from `.env` the Inspector link 404s on Supabase and dev views hang on "Compiling…". Open `http://localhost:3000/functions/v1/chef/mcp/inspector`, add the localhost MCP URL, Authenticate (the owner signs in at the consent site; never paste tokens into browser fields), Execute a view tool, then drive it with the Chrome tools. Keep the window ≥1200 px wide or the Inspector goes single-column; typing letters outside a field triggers its keyboard shortcuts.
...
- **Names first, ids second.** Anything a user can say gets a name argument (`recipe`, `item`, `member`, `candidates`, `participants`) resolved via `server/tools/resolve.ts`; the id twin stays for views. Ambiguity is a `ToolError` listing the matches (the agent asks), never a guess. Every tool declares `title` + `annotations` from `hints` in `results.ts` (read / create / idempotent / destructive; `openWorldHint` always false).
...
- Deploy: `deno task deploy` (`--prod` for the prod project). It builds with external views, uploads them to the public `views` Storage bucket, bundles the server with the edge shim into one module (`supabase/functions/chef/bundle.js`, not tracked) and deploys it (ADR 0005, 0006). Every request boots a fresh worker on this plan and the boot pays for the module graph: mcp-use as bare imports cost 1.4 s per request, the single bundle 0.2 s, so never deploy the unbundled form. Keep the function bundle small: inline views pushed it past 2.6 MB and deploys failed with a bare `deploy status 500`; the same 500 has also hit a 66 KB bundle once, so retry once before digging, but do not go back to inline for deploys. Inline builds (`deno task build`) are for tests and local work only. The deploy also stores the project's JWKS in the `CHEF_JWKS` secret so workers verify tokens offline (`server/auth.ts`); bare imports the built server keeps (`jose`, `mcp-use/oauth`) must be in `supabase/functions/chef/deno.json` or the bundle step fails with `not in import map`.
- **The website's household screens** (`site/src/household/`, at `household/{week,vote,shopping}` under the site base) are the presentational views wired to the tools over HTTP: `site/src/mcp.ts` sends the Supabase session token as the bearer, and the server allows CORS for the site origin and localhost only (`cors` in `server/index.ts`, ADR 0007). Mirror `server/views/<name>/view.tsx` when wiring a screen. See them signed in with `deno task snap:household` (needs the CORS deployed to dev). `site/src/mcp-meta.ts` holds the wire constants for both the site and the scripts.
